...
### 📊 Visual Representation: Integer Quantization Mapping
This diagram shows the scaling mapping that converts high-precision float values into memory-efficient integer formats (INT8/INT4).

```mermaid
flowchart LR
    Float["FP32 Value (4 Bytes)"] --> Quantize["Scale & Round Operation"]
    Quantize --> Int8["INT8 Value (1 Byte)"]
    Int8 --> Dequantize["Dequantize (Restore Scale)"]
    Dequantize --> ApproxFloat["Approximate FP32 Output"]
    ...
    class Quantize cpu;
    class Int8 memory;
    class Float,ApproxFloat,Dequantize system;
```
