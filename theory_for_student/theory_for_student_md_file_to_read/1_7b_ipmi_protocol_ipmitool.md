...
```mermaid
flowchart LR
    Admin[Admin Console\nRuns ipmitool] -->|UDP Port 623\nRMCP/RMCP+ Protocol| MgmtNIC[Dedicated Mgmt NIC]
    MgmtNIC --> BMC[BMC SoC]
    
    subgraph Target Server [Target Server Hardware]
        BMC <--> SDR[(Sensor Data Repository\nSDR)]
        BMC <--> SEL[(System Event Log\nSEL)]
        BMC <--> FRU[(Field Replaceable Unit\nFRU)]
        BMC -->|Chassis Control| Power[Power Relay / Fan PWM]
    end

    class Admin cpu;
    class MgmtNIC system;
    class BMC cpu;
    class SDR memory;
    class SEL memory;
    class FRU memory;
