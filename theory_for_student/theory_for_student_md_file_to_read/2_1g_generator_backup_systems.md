...
### 📊 Visual Representation: Power Backup Architecture
This diagram shows how Utility Grid power and Generator backup power interface through the Automatic Transfer Switch (ATS) to provide uninterrupted power to GPU servers via the UPS.

```mermaid
flowchart LR
    Grid["Utility AC Grid (Primary)"] -->|Normal Path| ATS{"Automatic Transfer Switch (ATS)"}
    Gen["Diesel Generator (Backup)"] -->|Backup Path| ATS
    ATS -->|Clean AC Power| UPS["UPS (Double-Conversion)"]
    UPS -->|Uninterrupted AC| IT["IT Rack (NVIDIA DGX GPU Servers)"]
