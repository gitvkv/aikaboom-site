...
### 📊 Visual Representation: OS Overhead Comparison

This diagram contrasts the resource footprint and software layers of a desktop installation with a minimal headless installation, showing how stripping the GUI maximizes resources for AI workloads.

```mermaid
flowchart TD
    subgraph Desktop["Desktop Installation (Resource Heavy)"]
        direction TB
        AppD["AI Workloads (PyTorch / CUDA)"]
        GUI["GNOME / KDE Desktop Environment\n(Uses 1-2 GB System RAM)"]
        XOrg["Display Server (X11 / Wayland)"]
        DCore["Core OS & Linux Kernel"]
    ...
    subgraph Minimal["Minimal / Headless Installation (Resource Lean)"]
        direction TB
        AppM["AI Workloads (PyTorch / CUDA)\n(Direct Access to Compute)"]
        MCore["Core OS & Linux Kernel"]
        
        AppM --> MCore
    end

    class AppD,AppM memory;
    class GUI,XOrg system;
    class DCore,MCore cpu;
