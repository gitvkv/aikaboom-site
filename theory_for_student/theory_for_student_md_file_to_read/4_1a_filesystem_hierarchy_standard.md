...
```mermaid
flowchart TD
    Root["/ (Root)"] --> System["System Configuration & Binaries"]
    Root --> User["User Space & Optionals"]
    Root --> Hardware["Virtual Filesystems & Devices"]

    System --> etc["/etc<br>System Configs"]
    System --> bin["/bin & /sbin<br>System Binaries"]
    System --> boot["/boot<br>Kernel & Bootloader"]

    User --> home["/home & /root<br>User Directories"]
    User --> usr["/usr<br>Libraries & Apps"]
    User --> opt["/opt<br>Third-party (CUDA)"]
    User --> var["/var<br>Variable Data (Logs)"]

    Hardware --> dev["/dev<br>Devices (e.g., nvidia0)"]
    Hardware --> proc["/proc<br>Virtual Process Info"]
    Hardware --> sys["/sys<br>Virtual System Info"]

    classDef cpu fill:#eafaf1,stroke:#76b900,stroke-width:2px,rx:6px,ry:6px;
    classDef memory fill:#f0f7ff,stroke:#3498db,stroke-width:1.5px,rx:4px,ry:4px;
    classDef system fill:#f1f5f9,stroke:#64748b,stroke-width:1.5px;

    class dev,proc,sys cpu;
    class home,var,opt memory;
    class Root,System,User,Hardware,etc,bin,boot,usr system;
```
