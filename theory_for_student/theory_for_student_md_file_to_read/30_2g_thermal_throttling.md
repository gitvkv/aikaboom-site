...
### 📊 Visual Representation: Thermal Throttling core slowdown
This flowchart demonstrates how high temperatures trigger driver limits to drop core clocks, protecting GPU silicon.

```mermaid
flowchart LR
    TempHigh["GPU temp exceeds 83°C (Slowdown temp)"] --> Throttling{"Driver Throttling State"}
    Throttling -->|Reduce| Clocks["Core Clocks scale down (Throttled)"]
    Clocks --> TempSafe["Silicon cools down (Compute bottlenecked)"]

    classDef cpu fill:#eafaf1,stroke:#76b900,stroke-width:2px,rx:6px,ry:6px;
    classDef memory fill:#f0f7ff,stroke:#3498db,stroke-width:1.5px,rx:4px,ry:4px;
    classDef system fill:#f1f5f9,stroke:#64748b,stroke-width:1.5px;

    class Throttling cpu;
    class Clocks,TempSafe memory;
    class TempHigh system;
```
