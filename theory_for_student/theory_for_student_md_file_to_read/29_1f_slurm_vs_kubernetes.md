...
### 📊 Visual Representation: Slurm HPC vs. Kubernetes Orchestration comparison
This diagram contrasts Slurm (batch scheduling run-to-completion tasks) with Kubernetes (reconciliation-loop microservices).

```mermaid
flowchart LR
    subgraph Slurm["Slurm HPC (Queue focus)"]
        Job["Batch Jobs"] -->|Submit| Queue["Queue (Static Allocation)"]
    end
    subgraph K8s["Kubernetes (Reconcile focus)"]
        Pod["Active Pods"] -->|Loop| Controller["Reconcile state loop"]
    end
