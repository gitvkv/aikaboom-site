...
```mermaid
flowchart TD
    subgraph SP["Single-Phase Power (Pulsed, Max ~15kW)"]
        GridSP["Single AC Phase (1 Hot, 1 Neutral)"] -->|Pulsed Flow| LoadSP["Standard Load (Desktop/Switch)"]
    end

    subgraph TP["Three-Phase Power (Continuous, 50kW - 1MW+)"]
        GridTP["Three AC Phases (3 Hots offset by 120°)"] -->|Continuous Flow| LoadTP["AI Rack (NVIDIA DGX Systems)"]
    end
    ...
    class LoadTP cpu;
    class LoadSP memory;
    class GridSP,GridTP,SP,TP system;
```
