...
### 📊 Visual Representation: Linux PID Namespace Isolation
This diagram displays how the Linux PID namespace maps container process IDs to separate physical PIDs on the host system.

```mermaid
flowchart LR
    subgraph Container["Container PID Namespace"]
        CProcess["App Process (PID 1)"]
    end
    subgraph Host["Host PID Namespace"]
        HProcess["Host process representation (PID 12450)"]
    end
    CProcess -->|Mapping translation| HProcess
