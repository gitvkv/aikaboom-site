...
### 📊 Visual Representation: Singularity Image Format (SIF) flat file
This diagram displays the SIF structure, compiling overlays and file systems into a single compressed, cryptographically signed file.

```mermaid
flowchart LR
    SIF["SIF File (.sif)"] --> Header["SIF Header (Metadata & Signatures)"]
    SIF --> RootFS["SquashFS File System Image (Read-Only)"]
    SIF --> Signature["PGP Signature block"]
    ...
    class SIF cpu;
    class Header,RootFS,Signature memory;
```
