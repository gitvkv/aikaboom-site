...
### 📊 Visual Representation: nvidia-smi Cheat Sheet commands
This diagram clusters essential nvidia-smi flags for query, monitoring, and active control operations.

```mermaid
flowchart LR
    nvidiaSmi["nvidia-smi Flags"] --> Query["Query (e.g. -q -d MEMORY)"]
    nvidiaSmi --> Monitor["Monitor (e.g. dmon / pmon)"]
    nvidiaSmi --> Control["Control (e.g. -pl / -pm / -lgc)"]
    ...
    class nvidiaSmi cpu;
    class Query,Monitor,Control memory;
```
