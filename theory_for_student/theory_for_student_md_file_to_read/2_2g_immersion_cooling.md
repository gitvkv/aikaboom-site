...
```mermaid
flowchart LR
    subgraph SP["Single-Phase Immersion"]
        ServerSP["Submerged GPU Server"] -->|Sensible Heat| LiquidSP["Liquid Dielectric Fluid (No Phase Change)"]
        LiquidSP -->|Pumped Out| HX_SP["External Heat Exchanger"]
        HX_SP -->|Pumped In| LiquidSP
    end

    subgraph TP["Two-Phase Immersion"]
        ServerTP["Submerged GPU Server"] -->|"Latent Heat (Boiling)"| VaporTP["Fluid Vapor (Phase Change)"]
        VaporTP -->|Rises to Top| Condenser["Ceiling Condenser Coil"]
        Condenser -->|"Condensation (Liquid)"| LiquidTP["Liquid Return (Drips Down)"]
        LiquidTP --> ServerTP
    end
    ...
    class ServerSP,ServerTP cpu;
    class LiquidSP,LiquidTP memory;
    class HX_SP,Condenser,VaporTP,SP,TP system;
```
