...
### 📊 Visual Representation: Single-Bit (SBE) vs. Double-Bit (DBE) ECC
This diagram displays how single-bit errors are corrected on the fly, while double-bit errors cause memory corruption and crash processes.

```mermaid
flowchart LR
    subgraph SBE["Single-Bit Error (Correctable)"]
        BitFlip1["Single Bit Flip"] -->|Hardware ECC correction| Safe["Data Restored (No crash)"]
    end
    subgraph DBE["Double-Bit Error (Uncorrectable)"]
        BitFlip2["Double Bit Flip"] -->|Parity check fails| Crash["Process Terminated (Xid 95 / Crash)"]
    end
