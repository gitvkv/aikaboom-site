...
### 📊 Visual Representation: On-Demand vs. Reserved vs. Spot pricing models
This diagram displays cost models, comparing On-Demand pricing with Reserved (discounted commitments) and Spot instances (unstable preemptible pools).

```mermaid
flowchart LR
    Pricing["GPU Pricing Models"] --> OnDemand["On-Demand (Highest cost / Zero commitment)"]
    Pricing --> Reserved["Reserved (1-3 Year commitments / 30-70% Discount)"]
    Pricing --> Spot["Spot/Preemptible (Lowest cost / Subject to preemption)"]
    ...
    class Pricing cpu;
    class OnDemand,Reserved,Spot memory;
```
