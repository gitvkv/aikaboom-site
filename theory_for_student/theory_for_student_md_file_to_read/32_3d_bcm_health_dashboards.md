...
### 📊 Visual Representation: BCM Node Health checking
This diagram displays how BCM periodically checks node health (temperatures, system services, logs) and reports status to dashboards.

```mermaid
flowchart LR
    NodeCheck["BCM Health Daemon"] --> CPUChecks["CPU / RAM / Disk usage checks"]
    NodeCheck --> GPUChecks["nvidia-smi / NVLink check"]
    NodeCheck --> UI["BCM Dashboard (Healthy / Warning / Failed)"]
    ...
    class NodeCheck cpu;
    class CPUChecks,GPUChecks memory;
    class UI system;
```
