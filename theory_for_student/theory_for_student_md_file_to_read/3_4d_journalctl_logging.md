...
```mermaid
flowchart TD
    subgraph LogSources["Log Producers (User & Kernel)"]
        direction LR
        KLog["Kernel (dmesg / GPU Drivers)"]
        USvc["User Services (nvidia.service, docker)"]
        App["AI App stdout/stderr"]
    end

    LogSources -->|Captured by| Daemon["systemd-journald Daemon"]
    
    subgraph Storage["Structured Binary Journal"]
        direction TB
        BJournal["/var/log/journal/\n(Metadata: Timestamp, Unit, Priority)"]
    end
    Daemon -->|Writes to| BJournal

    BJournal -->|Queried by| Exec["journalctl CLI"]

    subgraph Filters["journalctl Filtering Flags"]
        direction LR
        Time["--since / --until\n(Time Range)"]
        Unit["-u unit.service\n(Systemd Unit)"]
        Prio["-p priority\n(Log Level Severity)"]
    ...
    class KLog,USvc cpu;
    class BJournal,Storage memory;
    class Daemon,Exec,Time,Unit,Prio,Follow system;
