...
```mermaid
flowchart LR
    Query["Find Profile ID (e.g. 19)"] --> CreateGPU["1. Create GPU Instance (mig -cci 19)"]
    CreateGPU --> CreateCompute["2. Create Compute Instance (mig -cci 19 -cci 0)"]
    CreateCompute --> Verify["3. Check instances via nvidia-smi"]
    ...
    class CreateGPU cpu;
    class Query,CreateCompute,Verify system;
```
