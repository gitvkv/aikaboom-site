...
flowchart LR
    subgraph Host["Host Application Server"]
        CPU["Host CPU"] --- GPU["NVIDIA GPUs (Compute plane)"]
    end
    subgraph DPUPlane["BlueField DPU (Infrastructure plane)"]
        DPU["ARM Cores (OVS / Storage virtualizer)"]
    end
    Host ---|PCIe Bus| DPUPlane
