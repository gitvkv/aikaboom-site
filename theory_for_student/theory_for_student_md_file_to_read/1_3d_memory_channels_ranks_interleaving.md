...
### 📊 Visual Representation: Memory Interleaving and Channel/Rank Distribution

This diagram illustrates how the memory controller splits an incoming data stream into chunks and interleaves them across multiple channels and memory ranks in parallel.

```mermaid
flowchart TD
    subgraph Controller["CPU / GPU Memory Controller"]
        DataStream["Incoming Data Stream\n(Chunks: C1, C2, C3, C4, C5, C6...)"]
        Mux["Channel Interleaving Logic"]
        DataStream --> Mux
    end

    subgraph Ch1["Memory Channel A"]
        subgraph DIMM1["Dual-Rank DIMM"]
            Rank_A1["Rank 0 (Memory Bank 0)"]
            Rank_A2["Rank 1 (Memory Bank 1)"]
        end
    end

    subgraph Ch2["Memory Channel B"]
        subgraph DIMM2["Dual-Rank DIMM"]
            Rank_B1["Rank 0 (Memory Bank 0)"]
            Rank_B2["Rank 1 (Memory Bank 1)"]
        end
    end
    ...
    Ch1 -->|Rank Interleave| Rank_A1
    Ch1 -->|Rank Interleave| Rank_A2

    Ch2 -->|Rank Interleave| Rank_B1
    Ch2 -->|Rank Interleave| Rank_B2

    classDef cpu fill:#eafaf1,stroke:#76b900,stroke-width:2px,rx:6px,ry:6px;
    classDef memory fill:#f0f7ff,stroke:#3498db,stroke-width:1.5px,rx:4px,ry:4px;
    classDef system fill:#f1f5f9,stroke:#64748b,stroke-width:1.5px;

    class Controller,Mux cpu;
    class Rank_A1,Rank_A2,Rank_B1,Rank_B2,DIMM1,DIMM2 memory;
    class Ch1,Ch2,DataStream system;
```
