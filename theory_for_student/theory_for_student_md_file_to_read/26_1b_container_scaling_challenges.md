...
### 📊 Visual Representation: Manual Multi-Node Scaling Failures
This flowchart demonstrates scaling limits: manual load balancing, IP tracking, and node failure restarts saturate administrative work.

```mermaid
flowchart LR
    ScaleUp["Scale to 100 Containers"] --> IPTrack["1. Manual Port & IP allocation conflicts"]
    IPTrack --> NodeFail["2. Host crashes (Manually restart containers)"]
    NodeFail --> Outage["Throughput Drop / Service Outages"]
    ...
    class IPTrack cpu;
    class NodeFail memory;
    class ScaleUp,Outage system;
```
