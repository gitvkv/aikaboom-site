...
### 📊 Visual Representation: Infrastructure Foundations Exam concept mapping
This diagram displays the relationship between hardware components, power, and thermal limits tested in foundations exams.

```mermaid
flowchart LR
    Exam["Foundations Exam"] --> Power["Power Limits (TDP/PUE)"]
    Exam --> Temp["Thermal Throttling (Slowdown / Shutdown)"]
    Exam --> Components["Hardware (PCIe widths / Cores)"]
    ...
    class Exam cpu;
    class Power,Temp,Components memory;
```
