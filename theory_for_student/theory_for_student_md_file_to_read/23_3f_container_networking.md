...
### 📊 Visual Representation: Container Bridge and Host Networks
This diagram displays how the container Bridge network virtualizes IP addresses, while Host network mode routes traffic directly to physical server interfaces.

```mermaid
flowchart LR
    subgraph Bridge["Bridge Mode (Default)"]
        C1["Container 1 (IP: 172.17.0.2)"] -->|NAT| HostIP1["Host Interface (IP: 192.168.1.100)"]
    end
    subgraph HostMode["Host Mode"]
        C2["Container 2 (Direct Port)"] --> HostIP2["Host Interface (No virtual IP)"]
    end
