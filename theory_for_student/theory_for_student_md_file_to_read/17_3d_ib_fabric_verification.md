...
### 📊 Visual Representation: InfiniBand Fabric Verification Diagnostics
This flowchart maps fabric diagnostics: using standard tools to discover links, verify NDR rates, and log bit-error rates.

```mermaid
flowchart LR
    Discovery["ibnetdiscover (Map topology)"] -->|Verify ports| LinkCheck["iblinkinfo (Verify links)"]
    LinkCheck -->|Read registers| ErrorCheck["perfquery (Reset & verify bit errors)"]
    ...
    class LinkCheck cpu;
    class Discovery,ErrorCheck memory;
```
