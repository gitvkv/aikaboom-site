...
### 📊 Visual Representation: Standard Permissions vs. Granular ACLs
This diagram contrasts the rigid three-tier owner/group/other permission model with the flexible, multi-entity mapping offered by Access Control Lists (ACLs).

```mermaid
flowchart LR
    subgraph Standard["Standard UGO (1 Owner, 1 Group)"]
        owner["Owner (rwx)"]
        group["Group (r-x)"]
        other["Other (---)"]
    end

    subgraph ACL["ACL Granular (Multiple Users/Groups)"]
        acl_owner["Owner: Alice (rwx)"]
        acl_group["Group: datascience (r-x)"]
        acl_other["Other (---)"]
        acl_user1["User: Bob (r-x)"]
        acl_user2["User: Carol (rwx)"]
        acl_group2["Group: partners (r--)"]
    end
    ...
    class acl_user2,owner cpu;
    class acl_owner,acl_user1,group memory;
    class other,acl_group,acl_other,acl_group2 system;
```
