...
```mermaid
flowchart LR
    HeadNode["BCM Head Node"] --> CMD["CMD Daemon (Cluster management engine)"]
    HeadNode --> Services["DHCP / TFTP / DNS network services"]
    HeadNode --> DB["MariaDB (Cluster state logs)"]
