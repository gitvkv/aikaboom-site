...
### 📊 Visual Representation: Standard Attention vs. FlashAttention IO Paths
This diagram contrasts standard attention (writing large intermediate matrices to DRAM) with FlashAttention (tiled compute inside fast SRAM).

```mermaid
flowchart LR
    subgraph Standard["Standard Attention (DRAM Bound)"]
        DRAM_Load["DRAM Load"] --> HBM_Read["Read Q, K, V"] --> DRAM_Write["Write N x N Attention Matrix to DRAM"]
    end
    subgraph Flash["FlashAttention (SRAM Bound)"]
        SRAM_Load["SRAM Load"] --> SRAM_Tile["Tiled Compute in SM SRAM"] --> HBM_Write["Write Final Softmax Output to DRAM"]
    end
    ...
    class SRAM_Tile cpu;
    class DRAM_Write,HBM_Write memory;
    class DRAM_Load,SRAM_Load,HBM_Read system;
```
