...
### 📊 Visual Representation: CRI and OCI Container Runtimes
This diagram displays the runtime separation: the high-level runtime (containerd) communicates with low-level runtimes (runc/nvidia-container-runtime).

```mermaid
flowchart LR
    Kubelet["kubelet Agent"] -->|CRI gRPC| Containerd["containerd Runtime (High-level)"]
    Containerd -->|OCI Spec| NVRuntime["nvidia-container-runtime (Low-level)"]
    NVRuntime --> Container["Active Container process"]

    classDef cpu fill:#eafaf1,stroke:#76b900,stroke-width:2px,rx:6px,ry:6px;
    classDef memory fill:#f0f7ff,stroke:#3498db,stroke-width:1.5px,rx:4px,ry:4px;
    classDef system fill:#f1f5f9,stroke:#64748b,stroke-width:1.5px;

    class Containerd cpu;
    class NVRuntime memory;
    class Kubelet,Container system;
```
