...
```mermaid
flowchart LR
    eval["Bash Parser evaluates input"]
    eval --> single["Single Quotes"]
    eval --> double["Double Quotes"]
    eval --> sub["Command Sub: $()"]

    single -->|Process| lit["Literal string (No expansion)<br>'$VAR' -> '$VAR'"]
    double -->|Process| exp["Expand variables & command sub<br>'$VAR' -> value"]
    sub -->|Process| exec["Execute command & return stdout<br>'$(date)' -> Sun Jun 28"]
    ...
    class sub,exec cpu;
    class double,exp memory;
    class eval,single,lit system;
```
