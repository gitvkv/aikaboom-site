...
### 📊 Visual Representation: Kubernetes YAML Manifest fields
This diagram displays the 4 required top-level metadata keys in every Kubernetes YAML manifest.

```mermaid
flowchart LR
    Yaml["YAML Manifest"] --> ApiVersion["apiVersion: v1"]
    Yaml --> Kind["kind: Pod"]
    Yaml --> Metadata["metadata (Name, Labels)"]
    Yaml --> Spec["spec (Container images, volumes)"]
    ...
    class Yaml cpu;
    class ApiVersion,Kind,Metadata,Spec memory;
```
