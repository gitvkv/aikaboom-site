...
### 📊 Visual Representation: Hardware Chain-of-Custody Lifecycle
This diagram shows the step-by-step chain-of-custody tracking process for physical server hardware, audited continuously by access logs and CCTV surveillance.

```mermaid
flowchart LR
    Recv["1. Receiving Dock<br>(Serial Logged + CCTV)"] --> Stage["2. Secure Staging<br>(Badge Entry Logs)"]
    Stage --> Inst["3. Installation<br>(Sign-off + Video Audit)"]
    Inst --> Maint["4. Active Maintenance<br>(Parts Swapped + Logged)"]
    Maint --> Decom["5. Decommissioning<br>(Drive Wipe + Destruction Cert)"]

    classDef cpu fill:#eafaf1,stroke:#76b900,stroke-width:2px,rx:6px,ry:6px;
    classDef memory fill:#f0f7ff,stroke:#3498db,stroke-width:1.5px,rx:4px,ry:4px;
    classDef system fill:#f1f5f9,stroke:#64748b,stroke-width:1.5px;

    class Inst,Maint cpu;
    class Stage,Decom memory;
    class Recv system;
```
