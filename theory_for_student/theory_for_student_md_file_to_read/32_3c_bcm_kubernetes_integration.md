...
### 📊 Visual Representation: BCM Kubernetes cluster bootstrap
This flowchart shows how BCM deploys kubeadm to configure api-servers and join worker nodes automatically.

```mermaid
flowchart LR
    BCM["BCM Kubernetes wizard"] --> Master["Configure K8s Control Plane nodes"]
    Master -->|Autogen join scripts| Join["Workers join (Kubelet active)"]
    Join --> Operator["Deploy NVIDIA GPU Operator"]
