...
### 📊 Visual Representation: GPUDirect Storage Hardware compatibility
This diagram lists GDS prerequisites: matching PCIe switches (PLX), target NVMe/RoCE controllers, and custom kernel modules.

```mermaid
flowchart LR
    Prereq["GDS Requirements"] --> PCIe{"Common PCIe Switch (P2P enabled)"}
    Prereq --> Target["NVMe SSD / RoCE Fabric Controller"]
    Prereq --> Driver["nvfs Kernel Module Installed"]
    ...
    class Prereq cpu;
    class PCIe,Target,Driver memory;
```
