...
```mermaid
flowchart LR
    DOCA["DOCA Frameworks"] --> Flow["DOCA Flow (Packet processing)"]
    DOCA --> Telemetry["DOCA Telemetry (Switch/Link health)"]
    DOCA --> Storage["DOCA Firefly / Storage (Virtual disks)"]
    ...
    class DOCA cpu;
    class Flow,Telemetry,Storage memory;
```
