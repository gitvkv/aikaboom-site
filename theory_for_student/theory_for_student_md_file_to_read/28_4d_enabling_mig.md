...
### 📊 Visual Representation: nvidia-smi MIG Activation flow
This flowchart maps out enabling MIG mode: enabling the state, resetting the GPU, and verifying partition configs.

```mermaid
flowchart LR
    StopApps["1. Terminate GPU processes"] --> EnableMIG["2. nvidia-smi -i 0 -mig 1 (Enable)"]
    EnableMIG --> ResetGPU["3. nvidia-smi --gpu-reset"]
    ResetGPU --> Verify["4. nvidia-smi mig -lgip (List profiles)"]
    ...
    class EnableMIG cpu;
    class Verify memory;
    class StopApps,ResetGPU system;
```
