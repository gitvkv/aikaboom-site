...
```mermaid
flowchart LR
    Query["Conv Query (Filter & Input sizes)"] --> Engine{"cuDNN Heuristics Engine"}
    Engine -->|Analyze| AlgoA["Algorithm A (Best for small batch)"]
    Engine -->|Analyze| AlgoB["Algorithm B (Best for tensor cores)"]
