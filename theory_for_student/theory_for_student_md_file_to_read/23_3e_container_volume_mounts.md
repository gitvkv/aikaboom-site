...
flowchart LR
    subgraph HostFS["Host File System"]
        Folder["Host Folder: /data/dataset"]
        DockerArea["Docker Storage Area (/var/lib/docker/volumes/)"]
    end
    subgraph Container["Container Mounts"]
        Bind["Bind Mount (/app/data/)"] --> Folder
        Volume["Named Volume (/app/logs/)"] --> DockerArea
    end
    ...
    class Folder,DockerArea memory;
```
