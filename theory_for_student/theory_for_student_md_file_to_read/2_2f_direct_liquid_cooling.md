...
### 📊 Visual Representation: Direct Liquid Cooling (DLC) Architecture
This diagram shows the primary coolant loop flowing through the GPU cold plates and manifolds, interfacing with the facility's secondary chilled water loop via the CDU's heat exchanger.

```mermaid
flowchart LR
    subgraph Fac["Facility Loop"]
        Chiller["Central Chiller Plant"]
    end

    subgraph CDU_System["CDU (Coolant Distribution Unit)"]
        HX["Heat Exchanger"] <--> Pump["Pumps & Filters"]
    end

    subgraph Server_Rack["Server Rack (Primary Loop)"]
        SM["Supply Manifold"]
        RM["Return Manifold"]
        
        subgraph Server["GPU Server"]
            CP1["Cold Plate 1 (NVIDIA GPU)"]
            CP2["Cold Plate 2 (NVIDIA GPU)"]
        end
    end

    Chiller <-->|Secondary Loop| HX
    Pump -->|Cool Coolant| SM
    SM --> CP1 & CP2
    CP1 & CP2 -->|Warm Coolant| RM
    RM --> Pump
    ...
    class CP1,CP2 cpu;
    class SM,RM memory;
    class Chiller,HX,Pump,Fac,CDU_System,Server_Rack,Server system;
```
