...
### 📊 Visual Representation: libcufile API I/O Path
This diagram shows how userspace applications query GDS using the cuFile API, bypassing POSIX kernel buffering.

```mermaid
flowchart LR
    App["Userspace Application"] -->|cuFileRead / cuFileWrite| libcufile["libcufile.so Library"]
    libcufile -->|Direct DMA request| GDS_Driver["GDS Kernel Driver (nvfs)"]
    GDS_Driver -->|DMA| VRAM["GPU Memory (VRAM)"]

    classDef cpu fill:#eafaf1,stroke:#76b900,stroke-width:2px,rx:6px,ry:6px;
    classDef memory fill:#f0f7ff,stroke:#3498db,stroke-width:1.5px,rx:4px,ry:4px;
    classDef system fill:#f1f5f9,stroke:#64748b,stroke-width:1.5px;

    class App,libcufile cpu;
    class VRAM memory;
    class GDS_Driver system;
```
