...
```mermaid
flowchart LR
    yaml["YAML Config<br>(/etc/netplan/*.yaml)"] --> generate["netplan generate"]
    generate --> renderer{"netplan apply (Renderer)"}
    renderer -->|systemd-networkd| netd["systemd-networkd Daemon"]
    renderer -->|NetworkManager| nm["NetworkManager Daemon"]
