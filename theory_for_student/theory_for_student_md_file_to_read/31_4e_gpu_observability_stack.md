...
### 📊 Visual Representation: Unified GPU Observability Stack architecture
This diagram displays the full observability loop: from hardware GPUs, through dcgm-exporters, Prometheus, and Grafana.

```mermaid
flowchart LR
    GPU["Hardware GPUs"] -->|NVML| Exporter["dcgm-exporter (Port 9400)"]
    Exporter -->|Scrape| Prometheus["Prometheus server"]
    Prometheus -->|Query| Grafana["Grafana UI Dashboards"]

    classDef cpu fill:#eafaf1,stroke:#76b900,stroke-width:2px,rx:6px,ry:6px;
    classDef memory fill:#f0f7ff,stroke:#3498db,stroke-width:1.5px,rx:4px,ry:4px;
    classDef system fill:#f1f5f9,stroke:#64748b,stroke-width:1.5px;

    class Exporter,Prometheus cpu;
    class Grafana memory;
    class GPU system;
```
