...
### 📊 Visual Representation: kubectl GPU resource Cheat Sheet
This diagram clusters essential kubectl command scopes for monitoring operator pods, viewing node labels, and printing logs.

```mermaid
flowchart LR
    kubectl["kubectl Commands"] --> Operator["Operator pods (get pods -n gpu-operator)"]
    kubectl --> Nodes["Node status (describe node / labels)"]
    kubectl --> Logs["Logs (logs validation-pod)"]
    ...
    class kubectl cpu;
    class Operator,Nodes,Logs memory;
```
