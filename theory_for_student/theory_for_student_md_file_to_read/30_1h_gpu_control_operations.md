...
### 📊 Visual Representation: GPU Control configurations
This diagram outlines active hardware controls: setting persistent modes, power limits, and locking application clocks.

```mermaid
flowchart LR
    Control["nvidia-smi Commands"] --> Persistent["Persistence Mode (-pm 1 / Keeps driver loaded)"]
    Control --> PowerLimit["Power Limit (-pl 400 / Caps TDP)"]
    Control --> LockClocks["Lock Clocks (-lgc 1500 / Controls heating)"]
    ...
    class Control cpu;
    class Persistent,PowerLimit,LockClocks memory;
```
