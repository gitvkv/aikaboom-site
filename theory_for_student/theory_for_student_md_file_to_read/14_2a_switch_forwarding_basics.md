...
### 📊 Visual Representation: Layer 2 Switch MAC Address Learning
This diagram displays how switches learn source MAC addresses and populate forwarding tables to direct packets to target ports.

```mermaid
flowchart LR
    Incoming["Incoming Packet (Src MAC: A)"] --> Port["Switch Physical Port 1"]
    Port --> MACTable{"Store (MAC: A -> Port 1)"}
    MACTable --> Lookup{"Lookup Dest MAC"}
    Lookup -->|Found| Forward["Forward to associated port"]
    Lookup -->|Not Found| Flood["Flood to all other ports"]

    classDef cpu fill:#eafaf1,stroke:#76b900,stroke-width:2px,rx:6px,ry:6px;
    classDef memory fill:#f0f7ff,stroke:#3498db,stroke-width:1.5px,rx:4px,ry:4px;
    classDef system fill:#f1f5f9,stroke:#64748b,stroke-width:1.5px;

    class MACTable,Lookup cpu;
    class Incoming,Forward memory;
    class Flood system;
```
