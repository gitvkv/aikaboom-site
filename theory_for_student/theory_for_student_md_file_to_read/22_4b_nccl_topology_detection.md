...
### 📊 Visual Representation: NCCL hardware topology detection
This diagram displays how NCCL discovers physical connection paths (NVLink, PCIe, InfiniBand) to build the fastest communication graphs.

```mermaid
flowchart LR
    Init["NCCL Init"] --> Detect{"Probe Links"}
    Detect -->|Fast Path| NVLink["NVLink Mesh (Intra-node)"]
    Detect -->|Fallback| PCIe["PCIe / QPI Bus (Intra-node)"]
    Detect -->|Scale Out| IB["InfiniBand / RoCE (Inter-node)"]
