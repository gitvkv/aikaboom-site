...
### 📊 Visual Representation: Absolute vs. Relative Path Resolution
This flowchart contrasts absolute path resolution (starting from the system root `/`) and relative path resolution (originating from the current working directory).

```mermaid
flowchart LR
    subgraph AbsolutePath["Absolute Path (Starts from Root)"]
        root["/ (Root)"] --> home["home/"] --> user["ai_user/"] --> datasets["datasets/"] --> fileA["training_data.csv"]
    end

    subgraph RelativePath["Relative Path (Starts from CWD)"]
        cwd["Current Directory: /home/ai_user/projects/"] --> parent[".. (Parent: /home/ai_user/)"] --> datasetsRel["datasets/"] --> fileB["training_data.csv"]
    end
    ...
    class fileA,fileB cpu;
    class datasets,datasetsRel memory;
    class root,home,user,cwd,parent system;
```
