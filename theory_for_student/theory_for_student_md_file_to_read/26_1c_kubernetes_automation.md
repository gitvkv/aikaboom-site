...
### 📊 Visual Representation: Kubernetes Automation Loop
This diagram displays how Kubernetes loops: continually comparing actual running states to user-declared target manifests and resolving differences.

```mermaid
flowchart LR
    TargetState["User Declared State (Deploy 3 Pods)"] --> ControlLoop["K8s Reconcile Loop"]
    ControlLoop -->|Detect Delta| Actions["Auto-schedule new container pods"]
    Actions --> RunningState["Actual Cluster Running State"]
    RunningState --> ControlLoop

    classDef cpu fill:#eafaf1,stroke:#76b900,stroke-width:2px,rx:6px,ry:6px;
    classDef memory fill:#f0f7ff,stroke:#3498db,stroke-width:1.5px,rx:4px,ry:4px;
    classDef system fill:#f1f5f9,stroke:#64748b,stroke-width:1.5px;

    class ControlLoop cpu;
    class TargetState,RunningState memory;
