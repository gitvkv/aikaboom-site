...
### 📊 Visual Representation: NVIDIA Network Operator Control Loop
This diagram displays how the Network Operator automates Mellanox HCA drivers and RDMA CNI setups.

```mermaid
flowchart LR
    ClusterPolicy["Network Operator Config"] --> OFED["Deploy MOFED Drivers (Host Kernel)"]
    OFED --> CNI["Configure SR-IOV CNI & Multus"]
    CNI --> RDMA_Plugin["Deploy RDMA Device Plugin"]
    ...
    class OFED,CNI,RDMA_Plugin memory;
```
