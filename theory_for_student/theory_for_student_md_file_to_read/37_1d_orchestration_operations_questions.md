...
```mermaid
flowchart LR
    Exam["Orchestration Exam"] --> K8s["Kubernetes (GPU Operator / Scheduling)"]
    Exam --> Slurm["Slurm (Batch scripts / GRES partitions)"]
    Exam --> Operations["Monitoring (DCGM / metrics scraping)"]
    ...
    class Exam cpu;
    class K8s,Slurm,Operations memory;
```
