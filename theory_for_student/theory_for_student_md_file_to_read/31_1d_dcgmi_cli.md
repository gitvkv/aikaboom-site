...
### 📊 Visual Representation: dcgmi CLI subcommands
This diagram outlines the primary dcgmi groups used to manage groups, monitor fields, and view statuses.

```mermaid
flowchart LR
    dcgmi["dcgmi CLI utility"] --> Group["dcgmi group (Manage GPU sets)"]
    dcgmi --> FieldGroup["dcgmi fieldgroup (Select telemetry columns)"]
    dcgmi --> Health["dcgmi health (Manage alarm watches)"]
    ...
    class dcgmi cpu;
    class Group,FieldGroup,Health memory;
```
